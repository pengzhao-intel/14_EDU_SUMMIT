...
layer0 = ConvPoolLayer(rng, input=layer0_input,
                       ...
                       filter_shape=(32, 1, 3, 3), poolsize=(2, 2))
layer1_input = layer0.output.flatten(2)
layer1 = HiddenLayer(rng, input=layer1_input, n_in=32 * 13 * 13,
                     n_out=num_classes, activation=softmax)
...
for epoch in range(epochs):
    for batch_idx in range(n_train_batches):
        ...
        if ite % 50 == 0:
            ...
            print 'epoch %d iteration %d train loss %f \
            train acc %f test loss %f test acc %f' % \
                (epoch + 1, ite, train_loss, train_acc,
                 this_test_loss, this_test_acc)
